...
profile_name = 'All' # Your preferred port profile change to your chosen profile
...
def toggle_port(session, enable=True):
    ...
    print(f"{profile_name} has been {'enabled' if enable else 'disabled'} on Port {port_number} of switch {switch_mac}")

session = login()
toggle_port(session, enable=True)
